...
def find_anchor_left_obj_range(anchor, obj, detections):
    anchor_x = anchor['position'][0]
    obj_x = obj['position'][0]
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点左侧的、同类别的物体
    left_same_class = []
    for detection in detections:
        det_x = detection['position'][0]
        det_label = detection['class_name']
        if det_label == obj_label and det_x < anchor_x:
            left_same_class.append(detection)

    # 2. 按照 x 坐标从右到左排序
    left_same_class.sort(key=lambda d: d['position'][0], reverse=True)

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(left_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(left_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在左边同类中，返回 None
    return None
...
def find_anchor_right_obj_range(anchor, obj, detections):
    anchor_x = anchor['position'][0]
    obj_x = obj['position'][0]
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点右侧的、同类别的物体
    right_same_class = []
    for detection in detections:
        det_x = detection['position'][0]
        det_label = detection['class_name']
        if det_label == obj_label and det_x > anchor_x:
            right_same_class.append(detection)

    # 2. 按照 x 坐标从左到右排序
    right_same_class.sort(key=lambda d: d['position'][0])

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(right_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(right_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在右边同类中，返回 None
    return None

def find_anchor_image_above_obj_range(anchor, obj, detections):
    anchor_y = anchor['position'][1]
    obj_y = obj['position'][1]
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点上方的、同类别的物体
    above_same_class = []
    for detection in detections:
        det_y = detection['position'][1]
        det_label = detection['class_name']
        if det_label == obj_label and det_y < anchor_y:
            above_same_class.append(detection)

    # 2. 按照 y 坐标从下到上排序
    above_same_class.sort(key=lambda d: d['position'][1], reverse=True)

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(above_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(above_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在上边同类中，返回 None
    return None

def find_anchor_image_below_obj_range(anchor, obj, detections):
    anchor_y = anchor['position'][1]
    obj_y = obj['position'][1]
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点下方的、同类别的物体
    below_same_class = []
    for detection in detections:
        det_y = detection['position'][1]
        det_label = detection['class_name']
        if det_label == obj_label and det_y > anchor_y:
            below_same_class.append(detection)

    # 2. 按照 y 坐标从上到下排序
    below_same_class.sort(key=lambda d: d['position'][1])

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(below_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(below_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在下方同类中，返回 None
    return None

def find_anchor_world_above_obj_range(anchor, obj, detections, R):
    anchor_y = (np.array(anchor['position'], dtype=np.float64) @ R)[1]
    obj_y = (np.array(obj['position'], dtype=np.float64) @ R)[1]
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点上方的、同类别的物体
    above_same_class = []
    for detection in detections:
        det_y = (np.array(detection['position'], dtype=np.float64) @ R)[1]
        det_label = detection['class_name']
        if det_label == obj_label and det_y < anchor_y:
            above_same_class.append(detection)

    # 2. 按照 y 坐标从下到上排序
    above_same_class.sort(key=lambda d: (np.array(d['position'], dtype=np.float64) @ R)[1], reverse=True)
    ...
    # 4. 如果 obj 不在上方同类中，返回 None
    return None

def find_anchor_world_below_obj_range(anchor, obj, detections, R):
    anchor_y = (np.array(anchor['position'], dtype=np.float64) @ R)[1]
    obj_y = (np.array(obj['position'], dtype=np.float64) @ R)[1]
    obj_label = obj['class_name']
    ...
    # 2. 按照 y 坐标从上到下排序
    below_same_class.sort(key=lambda d: (np.array(d['position'], dtype=np.float64) @ R)[1])
...
def find_anchor_front_obj_range(anchor, obj, detections):
    anchor_min_z = np.min(np.array(anchor["corners"])[:, 2])
    obj_min_z = np.min(np.array(obj["corners"])[:, 2])
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点前方的、同类别的物体
    front_same_class = []
    for detection in detections:
        det_min_z = np.min(np.array(detection["corners"])[:, 2])
        det_label = detection['class_name']
        if det_label == obj_label and det_min_z < anchor_min_z:
            front_same_class.append(detection)

    # 2. 按照 z 坐标从后到前排序
    front_same_class.sort(key=lambda d: np.min(np.array(d["corners"])[:, 2]), reverse=True)

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(front_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(front_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在前方同类中，返回 None
    return None

def find_anchor_behind_obj_range(anchor, obj, detections):
    anchor_min_z = np.min(np.array(anchor["corners"])[:, 2])
    obj_min_z = np.min(np.array(obj["corners"])[:, 2])
    obj_label = obj['class_name']

    # 1. 获取所有位于锚点后方的、同类别的物体
    behind_same_class = []
    for detection in detections:
        det_min_z = np.min(np.array(detection["corners"])[:, 2])
        det_label = detection['class_name']
        if det_label == obj_label and det_min_z > anchor_min_z:
            behind_same_class.append(detection)

    # 2. 按照 z 坐标从前到后排序
    behind_same_class.sort(key=lambda d: np.min(np.array(d["corners"])[:, 2]))

    # 3. 遍历找到 obj 是第几个
    for idx, item in enumerate(behind_same_class):
        if np.allclose(item['position'], obj['position']):
            return 0 if len(behind_same_class) == 1 else (idx + 1)

    # 4. 如果 obj 不在后方同类中，返回 None
    return None

def find_max_variance_direction(A, B, C):
    objs = [A, B, C]

    # 提取各个方向的值
    x_vals = [obj['position'][0] for obj in objs]
    y_vals = [obj['position'][1] for obj in objs]
...
def sort_objects_by_direction(objs, direction):
    if direction == 'x':
        return sorted(objs, key=lambda obj: obj['position'][0])
    elif direction == 'y':
        return sorted(objs, key=lambda obj: obj['position'][1])
    elif direction == 'z':
        return sorted(objs, key=lambda obj: np.min(np.array(obj['corners'])[:, 2]))
    else:
        raise ValueError(f"Unknown direction: {direction}")
    
def check_ambiguity(middle, e1, e2, detections, direction):
    middle_class = middle['class_name']
    ...
    def get_val(obj):
        if direction == 'x':
            ...
            return np.min(np.array(obj['corners'])[:, 2])
    ...
    for det in detections:
        if det['class_name'] != middle_class:
            continue
        if np.allclose(det['position'], middle['position']):
            continue
        val = get_val(det)
        if lower < val < upper:
            return True  # 有歧义
