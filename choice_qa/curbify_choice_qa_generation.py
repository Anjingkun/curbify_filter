...
def check_and_clean_choice_qa(frame_dir: str, filename) -> str | None:
    """
    检查并尝试加载 frame_dir/choice_qa.json。
...
def clean_all_choice_qa(frame_dirs: list[str], choice_name):
    """
    批量检查并清理所有 choice_qa.json 文件。
    """
    print(f"🧹 Checking {len(frame_dirs)} folders...")

    for frame_dir in tqdm(frame_dirs):
        if check_and_clean_choice_qa(frame_dir, choice_name) is not None:
            print(f"✅ Cleaned {frame_dir}/{choice_name}")
    print("✅ Done.")
...
def find_unprocessed_frame_dirs(frame_dirs: list[str], filename) -> list[str]:
    """
    找出还没有生成 choice_qa.json 的帧目录
    """
    unprocessed = []
    for frame_dir in tqdm(frame_dirs, desc="🔍 Checking unprocessed frames"):
        choice_path = Path(frame_dir) / filename
        if not choice_path.exists():
            unprocessed.append(frame_dir)
    return unprocessed

# ✅ 假设你已经定义好了这个函数
def choice_qa_generation(frame_dir):
    """
    你已有的函数：生成 choice_qa.json 并保存到 frame_dir 中
    """
    ...
    with open(f"{frame_dir}/choice_qa.json", 'w') as f:
        json.dump(result, f, indent=4)


    
def process_all_unprocessed_dirs(unprocessed_dirs: list[str], num_workers: int = 8) -> list[str]:

    for unprocessed_dir in tqdm(unprocessed_dirs, desc="🔄 Processing unprocessed frames"):
        choice_qa_generation(unprocessed_dir)


# 示例用法
if __name__ == "__main__":
    root_dir = "/home_sfs/zhouenshen/dataset/3D/cubifyanything/filter_step_20"
    choice_name = "choice_qa.json"
    # ✅ 第一步：查找所有合法的帧文件夹（包含目标 JSON）
    frame_dirs = find_all_frame_dirs(root_dir)

    # ✅ 第二步：检查并清理损坏的 choice_qa.json 文件
    deleted_files = clean_all_choice_qa(frame_dirs, choice_name)

    # ✅ 第三步：找出还未处理的帧目录（没有 choice_qa.json）
    unprocessed_dirs = find_unprocessed_frame_dirs(frame_dirs, choice_name)

    # ✅ 第四步：多进程处理所有未处理帧
    processed_dirs = process_all_unprocessed_dirs(unprocessed_dirs)
